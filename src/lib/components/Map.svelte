<script lang="ts">
    import type { AccountData, CharacterData } from '$lib/db';
    import { Avatar, P, Tooltip } from "flowbite-svelte";

    // Accept players data as a prop
    let { players, activePlayer }: {players: AccountData[], activePlayer: string | null} = $props();
    ...
    const calculateScaleAndOffset = (samplePoints: MapPoint[]): {scaleX: number, scaleY: number, origoX: number, origoY: number} => {
    ...
    const mapToPixels = (player: CharacterData): MapPoint | undefined => {
        if (![0, 1].includes(player.map!)) return undefined;

        const scales = player.map === 1 ? kalimdorScaleAndOffset : eKScaleAndOffset;

        const px_x = (player.y * scales.scaleX * -1) + scales.origoX;
        const px_y = (player.x * scales.scaleY * -1) + scales.origoY;

        return {db_x: player.x, db_y: player.y, px_x, px_y, character: player};
    };
